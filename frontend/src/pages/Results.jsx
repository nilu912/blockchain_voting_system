...
const electionImg =
  "https://images.unsplash.com/photo-1494172961521-33799ddd43a5?ixlib=rb-4.0.3&auto=format&fit=crop&w=1470&q=80";
...
const Results = () => {
    ...
    const fetchCompletedElections = async () => {
      try {
        ...
        const completedElections = allElections.filter(
          (election) => election.isCompleted
        );
        // console.log(completedElections);
        // await new Promise((resolve) => setTimeout(resolve, 1000));

        const candidates = await contract.getAllCandidates(Number(completedElections[0].electionId));
        setCandidate(candidates)

        const winnerData = await contract.getCandidate(Number(completedElections[0].electionId), Number(completedElections[0].winnerId));
        ...
        const mockElections = [
          ...
          },
          {
            ...
            description:
              "Results for the neighborhood community board election",
            endDate: "September 30, 2023",
            totalVotes: 856,
            image:
              "https://images.unsplash.com/photo-1577985051167-0d49eec21977?ixlib=rb-4.0.3&auto=format&fit=crop&w=1470&q=80",
            category: "Community",
            candidates: [
              {
                id: 1,
  ...
  const fetchElection = async (e) => {
    try {
      ...
      // const filteredData = candidates.map((c)=>(
  ...
  return (
    <div className="min-h-screen bg-white p-6">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-4xl font-bold text-[#023047] mb-8 text-center">
          Election Results
        </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {elections.map((election) => (
              <div
                ...
                onClick={() => fetchElection(election)}
              >
                <div className="relative h-32">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4">
                    <h3
                      ...
                      style={{ color: "white", opacity: "90%" }}
                    >
                      {election.electionName}
                    </h3>
                    <p className="text-white/80 text-sm">
                      {election.totalVotes} votes •
                      {formateDate(election.endTime)}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div>
              ...
              <div className="p-8 text-white">
                ...
                <div className="flex flex-wrap gap-4">
                  ...
                  <div className="bg-white/10 backdrop-blur-sm rounded-lg px-4 py-2">
                    ...
                    <span className="text-xl font-bold text-white">
                      {formateDate(Number(selectedElection.endTime))}
                    </span>
                  </div>
                  {/* <div className="bg-white/10 backdrop-blur-sm rounded-lg px-4 py-2">
                    <span className="block text-sm text-white/70">
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {candidates
                        ...
                        .map((candidate) => {
                          const isWinner = (Number(candidate.candidateId) == Number(winner.candidateId))
                          //   candidate ===
                          ...
                          return (
                            <tr key={candidate.candidateId}>
                              ...
                              <td className="px-6 py-4 whitespace-nowrap">
                                <div className="w-[80%] bg-gray-200 rounded-full h-2.5 mb-1">
                                  <div
                                    className={`h-2.5 rounded-full ${
                                      isWinner ? "bg-green-500" : "bg-[#023047]"
                                    }`}
            ...
            {/* <div className="bg-blue-50 rounded-xl p-6 border border-blue-100 mb-8">
              ...
              <p className="text-gray-700 mb-4">
                These results are permanently recorded on the blockchain and
                cannot be altered. Each vote has been cryptographically verified
                for authenticity.
              </p>
              <div className="flex flex-wrap gap-4">
                <div className="bg-white rounded-lg p-4 shadow-sm flex-1">
                  ...
                  <p className="font-mono text-sm text-gray-800 truncate">
                    0x7f9e8d7c6b5a4e3f2d1c0b9a8e7d6c5b4a3f2e1d
                  </p>
                </div>
                <div className="bg-white rounded-lg p-4 shadow-sm flex-1">
                  <h4 className="text-sm font-medium text-gray-500 mb-1">
