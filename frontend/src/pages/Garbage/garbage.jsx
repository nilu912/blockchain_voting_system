...
const Elections = () => {
  ...
  useEffect(() => {
    // Mock data for elections instead of fetching from contract
    const mockElections = [
      {
        electionId: 1,
      ...
      },
      {
        ...
        startTime: new Date(Date.now() - 86400000 * 2).getTime() / 1000, // 2 days ago
        endTime: new Date(Date.now() + 86400000 * 5).getTime() / 1000, // 5 days from now
        isActive: true,
      ...
      },
      {
        ...
        startTime: new Date(Date.now() - 86400000 * 3).getTime() / 1000, // 3 days ago
        endTime: new Date(Date.now() + 86400000 * 10).getTime() / 1000, // 10 days from now
        isActive: true,
    ...
    const formatted = activeElections.map((election, index) => ({
      ...
      startTime: new Date(Number(election.startTime) * 1000).toLocaleString(),
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="text-gray-600 text-lg max-w-2xl mx-auto">
            Experience transparent, tamper-proof elections with our cutting-edge blockchain technology.
            <span className="block mt-2 text-[#023047] font-medium">Every vote is securely recorded.</span>
          </p>
        </div>
        ...
        {elections.length > 0 ? (
            ...
            {elections.map((election) => (
              ...
              >
                ...
                <div className="relative z-20 p-6 -mt-6">
                  <h2 className="text-2xl font-bold text-white mb-2 group-hover:text-gray-100 transition-colors">{election.name}</h2>
                  ...
                  <div className="flex justify-between items-center mb-4">
                    <div className="flex items-center">
                      <div className="flex -space-x-2">
                        {[...Array(3)].map((_, i) => (
                          <div key={i} className="w-8 h-8 rounded-full bg-gradient-to-r from-blue-500 to-indigo-500 border-2 border-white"></div>
                        ))}
                      </div>
                      <span className="text-white/70 text-sm ml-2">+{election.participants} participants</span>
                    </div>
                  ...
                  <div className="mb-6">
                    <div className="w-full bg-white/20 rounded-full h-2">
                      <div 
                        ...
                        style={{ width: `${getProgressPercentage(election.startTime, election.endTime)}%` }}
                      ></div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-6 text-sm">
                    <div className="bg-white/10 rounded-lg p-3">
                      ...
                      <span className="text-white">{election.startTime.split(',')[0]}</span>
                    </div>
                    <div className="bg-white/10 rounded-lg p-3">
                      <span className="block text-white/70">Ends</span>
                      <span className="text-white">{election.endTime.split(',')[0]}</span>
                    </div>
                  </div>
          ...
          <div className="bg-gradient-to-br from-[#023047] to-gray-700 backdrop-blur-sm rounded-xl p-16 text-center shadow-xl">
            ...
            <h3 className="text-2xl font-bold text-white mb-3">No Active Elections</h3>
            <p className="text-white/80 max-w-md mx-auto">The next round of elections is being prepared. Check back soon or subscribe to notifications.</p>
