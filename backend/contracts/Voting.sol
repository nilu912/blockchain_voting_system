...
contract Voting {
    ...
    function createElection(
        ...
        uint _startTime,
        uint _endTime
    ) public onlyAdmin isValidString(_electionName) {
        ...
        isValidAddress(_candidateAddress)
    {
        ...
        require(
            elections[_electionId].registrationPhase,
    ...
    function regVoter(
        ...
        uint _electionId,
        uint _candidateId
    ) public isValidString(_voterName) {
        uint candidateCount = elections[_electionId].totalCandidates;
        require(
            !voters[_electionId][msg.sender].isRegistered,
        ...
        require(
            elections[_electionId].registrationPhase,
            "Registration phase is not active!"
        );
    ...
    function castVote(uint _electionId, uint _candidateId) public {
        ...
        emit castVoteEvent(
            voters[_electionId][msg.sender].voterId,
            "voter casted vote"
        );
    }
    ...
    function activeElection(uint _electionId) public onlyAdmin {
        require(_electionId < electionCount, "Invalid Election Id");
        require(
    ...
    function deactiveElection(uint _electionId) public onlyAdmin {
        require(_electionId <= electionCount, "Invalid Election Id");
        require(
            elections[_electionId].isActive,
            "Election is already deactiveted!"
        );
    ...
    function activeRegistration(uint _electionId) public onlyAdmin {
        require(_electionId < electionCount, "Invalid Election Id");
        require(elections[_electionId].isActive, "election is deactivated!");
        require(
            !elections[_electionId].registrationPhase,
            "registration is already activated!"
        );
        elections[_electionId].registrationPhase = true;
        emit activeRegistrationEvent(
            _electionId,
            "Now registration phase is Active"
        );
    }

    function deactiveRegistration(uint _electionId) public onlyAdmin {
        require(_electionId < electionCount, "Invalid Election Id");
        require(elections[_electionId].isActive, "election is deactivated!");
        require(
            elections[_electionId].registrationPhase,
            "registration is already deactivated!"
        );
        elections[_electionId].registrationPhase = false;
        emit deactiveRegistrationEvent(
            _electionId,
            "Now registration phase is Deactivated"
        );
    }

    function activeVoting(uint _electionId) public onlyAdmin {
        require(_electionId < electionCount, "Invalid Election Id");
        require(elections[_electionId].isActive, "election is deactivated!");
        require(
            !elections[_electionId].registrationPhase,
        ...
        require(
            !elections[_electionId].votingPhase,
            "Voting phase is already activeted!"
        );
        require(
            elections[_electionId].totalCandidates > 1,
            "Must have more than one candidate!"
        );
        require(
            elections[_electionId].totalVoters > 1,
            "Must have more than one voter!"
        );
        elections[_electionId].votingPhase = true;
        emit activeVotingEvent(_electionId, "Now voting phase is activated");
    }

    function deactiveVoting(uint _electionId) public onlyAdmin {
        require(_electionId < electionCount, "Invalid Election Id");
        require(elections[_electionId].isActive, "election is deactivated!");
        require(
            !elections[_electionId].registrationPhase,
        ...
        require(
            elections[_electionId].votingPhase,
            "Voting already deactivated@"
        );
        elections[_electionId].votingPhase = false;
        emit deactiveVotingEvent(
            _electionId,
            "Now voting phase is Deactivated"
        );
    }

    function completeElection(uint _electionId) public onlyAdmin {
        require(_electionId <= electionCount, "Invalid Election Id");
        require(
            !elections[_electionId].isActive,
            "election is already active!"
